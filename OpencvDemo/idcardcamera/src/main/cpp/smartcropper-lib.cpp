...
vector<Point> scanPoint(Mat bitmap) {
    vector<Point> result;
...
JNICALL Java_com_wildma_idcardcamera_cropper_CropOverlayView_Scan
        (JNIEnv *env, jobject obj, jobject bitmap) {
    AndroidBitmapInfo info;
    void *pixels;
    jobjectArray jobjArr;
    jclass intArr = env->FindClass("[I");
    jobjArr = env->NewObjectArray(4* 2, intArr, nullptr);
    ...
    if (info.format == ANDROID_BITMAP_FORMAT_RGBA_8888) {
        ...
        std::vector<Point> scanPoints = scanPoint(runpic);

        jint pointarry[2];

        for (jint i = 0; i <4; i++){
            jintArray colArr = env->NewIntArray(2);
            pointarry[0] = scanPoints[i].x;
            pointarry[1] = scanPoints[i].y;
